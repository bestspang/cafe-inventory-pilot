  ...
  return <Sidebar className="border-r">
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              {filteredNavItems.map(item => <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton asChild disabled={item.isDisabled}>
                    <Link to={item.isComing ? '#' : item.path} className={cn("relative group", location.pathname === item.path && "text-sidebar-primary font-medium", item.isComing && "opacity-50 cursor-not-allowed")}>
                      <item.icon className="mr-2 h-5 w-5" />
                      <span>{item.title}</span>
                      {item.isComing && <span className="absolute -top-1 -right-1 bg-primary text-primary-foreground text-[8px] px-1 py-0.5 rounded-full">
                          SOON
                        </span>}
                    </Link>
                  </SidebarMenuButton>
